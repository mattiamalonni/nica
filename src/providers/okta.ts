...
export default {
  name: "okta",
  authorizationUrl: "https://dev-xxxxxxxx.okta.com/oauth2/v1/authorize",
  tokenUrl: "https://dev-xxxxxxxx.okta.com/oauth2/v1/token",
  ...
  async fetchProfile(accessToken: string): Promise<unknown> {
    const response = await fetch(
      "https://dev-xxxxxxxx.okta.com/oauth2/v1/userinfo",
      {
        headers: {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Okta user: ${response.statusText}`);
    }
